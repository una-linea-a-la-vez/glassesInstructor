...
RULE 5: FOUR MWDAT KEYS & DEVELOPER MODE
There is NO "App ID 0" developer bypass. The real fast path is Developer Mode, enabled in
the Meta AI app (NOT Meta View): Settings > App Info > tap App version 5 times > toggle >
Confirm. Attestation is not used in Developer Mode, so dummy credentials work; only one
third-party app may be registered at a time.
MWDAT requires ALL FOUR keys or MWDATCore aborts with "Partial attestation configuration
detected. ClientToken and/or teamID are missing.":
- MWDAT -> MetaAppID = "<App ID from wearables.developer.meta.com>"
- MWDAT -> ClientToken = "<autogenerated alongside MetaAppID>"
- MWDAT -> TeamID = "<Apple Team ID, e.g. 82LLK72ZPK>"
